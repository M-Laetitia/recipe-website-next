...
const tagSchema = z.object({
    name: z.string().min(3, { message: "Name must be at least 3 characters long" }).min(1,{ message: "Name is required" }),
});
...
const AddTag = () => {
// useForm : C'est un hook fourni par react-hook-form pour gérer le formulaire.
    const {
        register, // permet de lier les entrées de formulair aux champs gérés par react-hook-form
        handleSubmit, //  utilisée pour gérer l'envoi du formulaire, et elle prend la fonction onSubmit en paramètre.
    ...
    const onSubmit = async (formData: TagSchema) => {
        try {
          ...
          const response = await fetch("/api/tag", { // Cette requête envoie les données au serveur en utilisant la méthode POST.
            method: "POST",
            headers: {
